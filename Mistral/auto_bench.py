...
python auto_bench.py \
  ...
  --max-new-tokens 2048 \
  --out-dir result/Mis_greedy/result \
  --model-path /root/autodl-tmp/models/Mistral-Small-3.1-24B-Instruct-2503 \
  --sem-layer-mapping /root/autodl-tmp/gemma2/map/best_layer_Mis_greedy.json \
...
python auto_bench.py \
  ...
  --samples-per-task 200 \
  --max-new-tokens 2048 \
  --out-dir result/Mis_sample/result \
  --model-path /root/autodl-tmp/models/Mistral-Small-3.1-24B-Instruct-2503 \
  --sem-layer-mapping map/best_layer_Mis_sample.json \
  --sem-retrieval-topk 10 \
