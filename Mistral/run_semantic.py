...
def _needs_mistral_regex_fix(model_path: str) -> bool:
    ...
    return "mistral" in name  # covers mistral / mistral-small / ministral / mistral3 ...


def _safe_from_pretrained(cls, model_path: str, **kwargs):
    """
    Robust from_pretrained across transformers versions:
    - dtype vs torch_dtype
    - fix_mistral_regex may not exist on older versions
    - use_fast may not exist
    """
    try:
        ...
        # 1) dtype -> torch_dtype fallback
        if "dtype" in kwargs and "unexpected keyword argument" in msg and "dtype" in msg:
            kw = dict(kwargs)
            kw["torch_dtype"] = kw.pop("dtype")
            return cls.from_pretrained(model_path, **kw)

        # 2) drop fix_mistral_regex if unsupported
        if "fix_mistral_regex" in kwargs and ("fix_mistral_regex" in msg and "unexpected keyword argument" in msg):
            kw = dict(kwargs)
            kw.pop("fix_mistral_regex", None)
            return cls.from_pretrained(model_path, **kw)

        # 3) drop use_fast if unsupported
        if "use_fast" in kwargs and ("use_fast" in msg and "unexpected keyword argument" in msg):
            kw = dict(kwargs)
...
def get_stop_token_ids(model, tokenizer) -> List[int]:
    ...
    # fallback: try eos_token string or </s> or id=2
    if not ids:
    ...
    # remove duplicates + filter BOS only
...
def load_model_and_tokenizer(
    ...
    # 1) processor (chat template often here on Mistral3)
    processor = None
    try:
    ...
    # 2) tokenizer (reuse processor.tokenizer if exists)
    tokenizer = None
    ...
    # ensure pad exists (but DO NOT use pad in stop logic)
    ...
    # 4) model: prefer AutoModelForImageTextToText (Mistral3), fallback to CausalLM
    ...
    return tokenizer, model, device, chat
...
def encode_prompt(
    ...
    """
    Two-stage: chat_template -> string -> tokenizer().
    Works better for Mistral3/Phi-3.
    """
...
def crop_past_key_values(past_kv, max_length: int):
    ...
    # ✅ generic cache types (newer transformers)
    if hasattr(past_kv, "crop") and callable(getattr(past_kv, "crop")):
...
def _safe_forward(model, **kwargs):
    """
    Some wrappers may not accept output_hidden_states; drop if needed.
    We rely on forward hook, so output_hidden_states is NOT required.
    """
    try:
        return model(**kwargs)
    except TypeError as e:
        msg = str(e)
        if "output_hidden_states" in kwargs and "unexpected keyword argument" in msg and "output_hidden_states" in msg:
            kw = dict(kwargs)
...
# ======================
# One-layer hidden capture (hook)
# ======================

def _get_transformer_layers(model):
    """
    Expand candidates to cover wrapper models like AutoModelForImageTextToText:
    ...
    candidates = [
        ...
        ("language_model.model.decoder.layers", lambda m: m.language_model.model.decoder.layers),
        ("model.model.layers", lambda m: m.model.model.layers),
        ("model.layers", lambda m: m.model.layers),
        ("model.model.layers", lambda m: m.model.layers),  # some models use model.layers directly
    ...
    for name, getter in candidates:
        try:
            ...
            continue
    raise RuntimeError(
        "Cannot locate transformer layers list for hook capture. "
        "Please extend _get_transformer_layers() for your model wrapper."
    )
...
def semantic_sd_forward_one_prompt(
    ...
    out = _safe_forward(model, **enc, use_cache=True, output_hidden_states=False)
    ...
    last_logits = getattr(out, "logits")[:, -1, :]  # [1,V]
    ...
    past_kv = crop_past_key_values(getattr(out_first, "past_key_values", None), cur_len)
    last_logits = getattr(out_first, "logits")[:, -1, :]
    ...
    while generated < max_new_tokens:
        ...
        if not hbuf.has_capacity(1 + K_max) or (cur_len + 1 + K_max >= ids_buf.size(1)):
            ...
            while generated < max_new_tokens:
                ...
                past_kv = crop_past_key_values(getattr(out_g, "past_key_values", None), cur_len)
                last_logits = getattr(out_g, "logits")[:, -1, :]
                generated += 1
        ...
        if hbuf.length > 1 and remaining > 1:
            q_norm = hbuf.buf_norm[hbuf.length - 1]  # [H]
            ...
            if pos.numel() > 0:
                ...
                start_draft = int((hist_next_idx + 1).item())  # j+2
        ...
        logits_new = getattr(out_merged, "logits")      # [1,L,V]
        past_kv_new = getattr(out_merged, "past_key_values", None)
        ...
        if mismatch and mismatch_token_t is not None:
            ...
            out_fix = _safe_forward(
                model,
                input_ids=mismatch_token_t,
            ...
            past_kv = crop_past_key_values(getattr(out_fix, "past_key_values", None), cur_len)
            last_logits = getattr(out_fix, "logits")[:, -1, :]
            generated += 1
...
def run_semantic_sd(args):
    set_seed(args.seed)
    tokenizer, model, device, chat = load_model_and_tokenizer(args.model_path, args.gpu_id)
    ...
    print(f"[Config] retrieval_topk={args.retrieval_topk} (similarity candidates; NOT sampling top_k)")

    print("\n[Warmup] Semantic SD ...")
    warmup_enc = encode_prompt(tokenizer, chat, "Warmup.", device)
    with torch.no_grad():
        _ = _safe_forward(model, **warmup_enc, use_cache=True, output_hidden_states=False)
    if torch.cuda.is_available():
        torch.cuda.synchronize()
    print("[Warmup done]\n")
