...
"""
Prompt Lookup Decoding (PLD) Benchmark on Spec-Bench using HuggingFace Native Interface.
**Sampling Version** - Supports temperature and top-p sampling.

Features:
1.  **Logic**: Uses HF `prompt_lookup_num_tokens` & `max_matching_ngram_size`.
2.  **Timing**: Uses `threading` + `TextIteratorStreamer` to precisely separate Prefill vs Decode.
3.  **Metrics**: Throughput = (NewTokens - 1) / DecodeTime.
4.  **Sampling**: Supports temperature, top_p, top_k for sampling-based generation.
...
python run_pld.py \
  ...
  --top_p 0.9 \
  --top_k 0 \
  --output result/ar_summarizaiton_sampling.jsonl
