...
python run_SRSD.py \
  ...
  --K 16 \
  --sim-threshold 0.0 \
  --bucket-max-per-token 512 \
  --bucket-query-cap 2048 \
...
class NextTokenAnchorBuckets:
    def __init__(self, max_per_token: int = 4096, query_cap: int = 2048):
        self.max_per_token = int(max_per_token)
        self.query_cap = int(query_cap)
        self.buckets = defaultdict(lambda: deque(maxlen=self.max_per_token))

    def init_from_tokens_cpu(self, tokens_cpu: List[int]):
        for new_pos in range(1, len(tokens_cpu)):
            anchor = new_pos - 2
            if anchor >= 0:
                token_prev = int(tokens_cpu[new_pos - 1])
                self.buckets[token_prev].append(anchor)

    def update_after_append_cpu(self, tokens_cpu: List[int], new_pos: int):
        if new_pos < 1:
            return
        anchor = new_pos - 2
        if anchor < 0:
    ...
    def get_recent_anchors(self, token_id: int) -> Optional[List[int]]:
        dq = self.buckets.get(int(token_id), None)
        if not dq:
            return None
        if self.query_cap > 0 and len(dq) > self.query_cap:
            return list(dq)[-self.query_cap:]
        return list(dq)
...
def semantic_sd_forward_one_prompt(
    ...
    top_p: float = 1.0,
    top_k: int = 0,
    bucket_max_per_token: int = 4096,
    bucket_query_cap: int = 2048,
):
    device = enc["input_ids"].device
    prompt_len = int(enc["input_ids"].shape[1])

    tokens_cpu: List[int] = enc["input_ids"][0].detach().cpu().tolist()
    ...
    if cap.hidden is None:
        raise RuntimeError("LayerCapture did not capture hidden in prefill.")
    ...
    u_prompt = hp / hp_norm
    mu_unit = u_prompt.mean(dim=0)
    scale = hp_norm.mean()
    center_1d = mu_unit * scale

    hbuf = NormHiddenBuffer(max_capacity_tokens, hidden_dim, device, norm_dtype, center_1d=center_1d)
    hbuf.init_from_prompt(h_prompt)

    buckets = NextTokenAnchorBuckets(max_per_token=bucket_max_per_token, query_cap=bucket_query_cap)
    buckets.init_from_tokens_cpu(tokens_cpu)
    ...
    base_id = int(base_token_t.item())
    ids_buf[0, cur_len:cur_len + 1] = base_tensor[0]
    cur_len += 1
    tokens_cpu.append(base_id)
    buckets.update_after_append_cpu(tokens_cpu, new_pos=len(tokens_cpu) - 1)
    ...
    while generated < max_new_tokens:
        ...
        if not hbuf.has_capacity(1 + K_max) or (cur_len + 1 + K_max >= ids_buf.size(1)):
            while generated < max_new_tokens:
                ...
                out_g = model(base_tensor, past_key_values=past_kv, use_cache=True, output_hidden_states=False)
                N_forward_decode += 1
                if cap.hidden is None:
                    raise RuntimeError("LayerCapture did not capture hidden in AR fallback.")
        ...
        if hbuf.length > 2 and remaining > 1:
            anchors_list = buckets.get_recent_anchors(base_id)
            if anchors_list:
                anchors = torch.tensor(anchors_list, device=device, dtype=torch.long)
                max_anchor = cur_len - 3
                if max_anchor >= 0:
                    anchors = anchors[anchors <= max_anchor]
                else:
                    anchors = anchors[:0]
                ...
                if anchors.numel() > 0:
                    ...
                    keys = hbuf.buf_norm.index_select(0, anchors)  # [M,H]
                    sims = torch.mv(keys, q_norm)                  # [M]
                    best_val, best_pos = torch.max(sims, dim=0)
                    if float(best_val.item()) >= float(sim_threshold):
                        best_anchor = anchors[best_pos]
                        start_draft = int(best_anchor.item()) + 2
        ...
        hidden_new = cap.hidden.squeeze(0)
        ...
        if mismatch and mismatch_token_t is not None:
            ...
            if n_draft_acc > 0 and start_draft >= 0:
                ids_buf[0, cur_len:cur_len + n_draft_acc] = draft_tokens_tensor[0, :n_draft_acc]
                draft_cpu = tokens_cpu[start_draft:start_draft + n_draft_acc]
                start_pos = len(tokens_cpu)
                tokens_cpu.extend(draft_cpu)
                for p in range(start_pos, len(tokens_cpu)):
                    buckets.update_after_append_cpu(tokens_cpu, new_pos=p)
                cur_len += n_draft_acc
            ...
            mismatch_id = int(mismatch_token_t.item())
            ids_buf[0, cur_len:cur_len + 1] = mismatch_token_t[0]
            cur_len += 1
            tokens_cpu.append(mismatch_id)
            buckets.update_after_append_cpu(tokens_cpu, new_pos=len(tokens_cpu) - 1)
...
def run_semantic_sd(args):
    ...
    print(f"[Config] Scheme A buckets: max_per_token={args.bucket_max_per_token}, query_cap={args.bucket_query_cap}")
    print("[Config] Retrieval: argmax similarity within bucket (no top-R/topk)")
        ...
        for task in tasks:
            ...
            for idx, prompt in enumerate(prompts, start=1):
                ...
                out_ids, stats, prompt_len = semantic_sd_forward_one_prompt(
                    ...
                    top_p=args.top_p,
                    top_k=args.top_k,
                    bucket_max_per_token=args.bucket_max_per_token,
                    bucket_query_cap=args.bucket_query_cap,
                )
    ...
    print("\n" + "=" * 60)
    print("Semantic SD Summary (Scheme A: next-token buckets)")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bucket-max-per-token", type=int, default=4096)
    parser.add_argument("--bucket-query-cap", type=int, default=2048)
