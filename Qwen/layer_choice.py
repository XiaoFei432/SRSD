...
def analyze_model_layers_fast(
    ...
    for task in ALL_SPEC_BENCH_CATEGORIES:
        ...
        for prompt in prompts:
            ...
            # ==========================================================
            # Fixed Mean Centering: compute μ once per layer, then
            # center + normalize. Retrieval is simple dot product.
            # ==========================================================
            all_hiddens_f = all_hiddens.float()  # [L, T, H]
            
            hp = all_hiddens_f[:, :prompt_len, :]                    # [L, Tp, H]
            hp_norm = hp.norm(dim=-1, keepdim=True) + 1e-8           # [L, Tp, 1]
            u = hp / hp_norm                                         # [L, Tp, H]
            mu_unit = u.mean(dim=1, keepdim=True)                    # [L, 1, H]
            scale = hp_norm.mean(dim=1, keepdim=True)                # [L, 1, 1]
            mu = mu_unit * scale                                     # [L, 1, H]
            #   [L, 1, H] # [L, 1, H]
            
            # Center then normalize
            centered = all_hiddens_f - mu  # [L, T, H]
            keys_norm = centered / (centered.norm(dim=-1, keepdim=True) + 1e-8)  # [L, T, H]

            # sim_matrix: [L, T, T] - simple dot product (same speed as original!)
...
def plot_semantic_curves_beautiful(
    ...
    """
    Generate a beautiful plot similar to the reference image.
    
    Features:
    - Smooth curves with Gaussian filter
    - Semi-transparent fill under curves
    - Different markers for best layer
    - Clean annotations
    ...
    for idx, task in enumerate(tasks):
        ...
        # Mark the best layer with a larger marker
        ...
        # Get PLD baseline for this task
    ...
    # Add horizontal line for reference (y=1, pure AR baseline)
    ...
    # Add subtle background gradient effect (optional)
