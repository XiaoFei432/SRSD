...
def semantic_sd_forward_one_prompt(
    ...
    while generated < max_new_tokens:
        ...
        # Step 2: Top-K retrieval (same logic)
        ...
        if hbuf.length > 1 and remaining > 1:
            ...
            top_vals, top_idxs = torch.topk(sims, k=k, largest=True, sorted=True)

            sim_ok = top_vals >= float(sim_threshold)

            hist_next = top_idxs + 1
            # no cur_len_t tensor: tensor<int compare
            ...
            if pos.numel() > 0:
                ...
                # one sync only after candidate exists (needed for slicing)
