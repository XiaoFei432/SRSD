...
ALGORITHM: identical to your current “mean-centering + cosine(dot)” SD
- Still computes a fixed center from prompt hiddens: center_1d
- Still stores normalized (centered) vectors, retrieval is dot product
- Still does topk(sim) then pick FIRST valid candidate (by similarity order)
- Still token-gates on hist_next token == base_token
- Still merged-forward verification and (if sampling) KV-fix for mismatch

Added (minimal, no semantic change):
- --disable-stop: set stop_ids_tensor empty => no early stop (force fixed-length generation)
- --window-size: restrict retrieval candidates to last W positions; -1 means global (no window)

Example:
python long_semantic.py \
  ...
  --window-size -1 \
  --disable-stop \
  --output semantic_code_edit_global_nostop.jsonl 
"""
...
def _get_transformer_layers(model):
    ...
    for name, getter in candidates:
        try:
            ...
            continue
    raise RuntimeError(
        "Cannot locate transformer layers list. Please add your model's path in _get_transformer_layers()."
    )
...
# ======================
# Normalized Buffer (Fixed Mean Centering) + last_norm cache
# ======================
...
class NormHiddenBuffer:
    ...
    Fixed Mean Centering (optional):
    - If center_1d is provided, subtract it BEFORE L2-normalize
    - Retrieval stays dot product

    Optimization:
    - Cache last_norm (last stored normalized vector).
    """
    def __init__(
        self,
        max_capacity: int,
        ...
        center_1d: Optional[torch.Tensor] = None,
    ):
        ...
        self.length = 0
        self.last_norm: Optional[torch.Tensor] = None

        self.center = None
        if center_1d is not None:
            self.center = center_1d.to(device=device, dtype=torch.float32)

    def _normalize_to_store_dtype(self, x_2d: torch.Tensor) -> torch.Tensor:
        xf = x_2d.float()
        if self.center is not None:
            xf = xf - self.center.unsqueeze(0)
    ...
    def init_from_prompt(self, hidden_2d: torch.Tensor):
        ...
        normed = self._normalize_to_store_dtype(hidden_2d)
        self.buf_norm[:n] = normed
        self.length = n
        self.last_norm = normed[-1]
    ...
    def append(self, hidden_2d: torch.Tensor):
        ...
        normed = self._normalize_to_store_dtype(hidden_2d)
        self.buf_norm[start:end] = normed
        self.length = end
        self.last_norm = normed[-1]
...
def semantic_sd_forward_one_prompt(
    ...
    window_size: int = -1,          # NEW: -1 global, >0 last W
    ...
    # ---- fixed mean center from prompt (same as your current) ----
    hp = h_prompt.float()  # [T,H]
    hp_norm = hp.norm(dim=-1, keepdim=True) + 1e-8
    u_prompt = hp / hp_norm
    mu_unit = u_prompt.mean(dim=0)  # [H]
    scale = hp_norm.mean()          # scalar
    center_1d = mu_unit * scale     # [H]

    hbuf = NormHiddenBuffer(max_capacity_tokens, hidden_dim, device, norm_dtype, center_1d=center_1d)
    ...
    # if stop_ids_tensor empty (disable-stop), this will always be False
    ...
    # pre-alloc to avoid torch.cat every step
    input_buf = torch.empty((1, 1 + K_max), dtype=ids_buf.dtype, device=device)
    ...
    while generated < max_new_tokens:
        ...
        # Step 2: Top-K retrieval (same logic; NEW: windowed range)
        draft_tokens_tensor = None
        K_candidate = 0

        if hbuf.length > 1 and remaining > 1:
            q_norm = hbuf.last_norm
            if q_norm is None:
                q_norm = hbuf.buf_norm[hbuf.length - 1]

            upto = hbuf.length - 1  # exclude current position
            if window_size is None or int(window_size) < 0:
                lo = 0
            else:
                w = int(window_size)
                lo = 0 if w <= 0 else max(0, upto - w)

            keys = hbuf.buf_norm[lo:upto]  # [T',H]
            sims = torch.mv(keys, q_norm)  # [T']

            rK = max(1, int(retrieval_topk))
            k = min(rK, int(sims.numel()))
            top_vals, top_idxs_local = torch.topk(sims, k=k, largest=True, sorted=True)

            # map back to global indices in full buffer
            top_idxs = top_idxs_local + lo  # [k]

            sim_ok = top_vals >= float(sim_threshold)

            hist_next = top_idxs + 1
            has_tail = (hist_next + 1) < cur_len
            in_range = hist_next < cur_len

            hist_next_clamped = hist_next.clamp(min=0, max=max(cur_len - 1, 0))
            hist_tokens = ids_buf[0, hist_next_clamped]
            ...
            if pos.numel() > 0:
                j = pos[0, 0]
                hist_next_idx = hist_next[j]  # scalar tensor

                start_draft = int((hist_next_idx + 1).item())
        ...
        # Step 3: merged forward (no torch.cat)
        if K_candidate > 0 and draft_tokens_tensor is not None:
            input_buf[:, 0:1] = base_tensor
            input_buf[:, 1:1 + K_candidate] = draft_tokens_tensor
            input_tokens = input_buf[:, :1 + K_candidate]
            draft_attempts += 1
        else:
...
def run_semantic_sd(args):
    ...
    if args.disable_stop:
        stop_ids = []
        stop_ids_tensor = torch.empty((0,), device=device, dtype=torch.long)
    else:
        stop_ids = get_stop_token_ids(model, tokenizer)
        stop_ids_tensor = torch.tensor(stop_ids, device=device, dtype=torch.long) if stop_ids else \
                          torch.empty((0,), device=device, dtype=torch.long)

    print(f"\n[Config] Stop token IDs: {stop_ids} (disabled={args.disable_stop})")
    ...
    print(f"[Config] window_size={args.window_size} (-1=global, W>0=last W)")
    print("[Config] Representation: mean-centering + L2-normalize, retrieval=dot")
        ...
        for task in tasks:
            ...
            for idx, prompt in enumerate(prompts, start=1):
                ...
                rec = {
                    ...
                    "disable_stop": args.disable_stop,
                ...
                print(
                    ...
                    f"Raw: {raw_new} | Eff: {eff_new} | TP: {throughput:.2f} tok/s | "
    ...
    print("\n" + "=" * 60)
    print("Semantic SD Summary (mean-centering + dot)")
    print("=" * 60)
    ...
    print(f"Window Size         : {args.window_size} (-1=global, W>0=last W)")
    print(f"Disable Stop        : {args.disable_stop}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--window-size", type=int, default=-1,
                        help="Retrieval window size. -1 means global. W>0 restricts to last W positions.")
    parser.add_argument("--disable-stop", action="store_true", default=False,
                        help="Disable early stop on EOS/stop tokens; force fixed-length generation.")
